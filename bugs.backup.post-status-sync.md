...
- Canonical tracked bugs (`BZ-001` .. `BZ-032`): `32/32` verified.
- Open canonical bugs: `0`.
- Canonical bugs pending QA: `0`.
- Legacy seed entries:
  - `BUG-001` -> mapped to `BZ-001` (verified)
...
## Batch C Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-010` Cash positions page now edits/deletes by unique `position.id` (row-level) instead of currency-wide fanout.

### Verification Executed
- `npx eslint src/app/cash/positions/page.tsx` (pass)

## Batch D Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-013` `buy_position` execution now enforces non-zero derived amount for `totalCost`-only flows (no zero-amount writes).
- `BZ-014` `add_cash` now resolves to a canonical manual account id (or creates a manual account) and uses consistent cash position naming.
- `BZ-015` `update_cash` now uses deterministic matching (`matchedPositionId`/exact manual account + currency) and returns ambiguity errors instead of fuzzy updates.
- `BZ-016` Chat query filtering now uses canonical asset-class semantics (`isPositionInAssetClass`) instead of legacy type-only checks.

### Verification Executed
- `npx eslint src/app/api/chat/route.ts` (pass with pre-existing warning)
- `npm run test -- src/services/domain/action-mapper.test.ts src/services/domain/portfolio-calculator.test.ts src/store/portfolioStore.test.ts src/services/domain/account-role-service.test.ts src/services/domain/cash-account-service.test.ts` (pass)

## Batch E Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-019` Partial sell REST route now delegates to domain `executePartialSell`, so remaining amount/cost-basis and transaction cost-basis are consistent with core domain rules.

### Verification Executed
- `npm run test -- src/services/domain/position-operations.test.ts src/services/domain/action-mapper.test.ts src/services/domain/portfolio-calculator.test.ts src/store/portfolioStore.test.ts src/services/domain/account-role-service.test.ts src/services/domain/cash-account-service.test.ts` (pass)
- `npx eslint src/app/api/portfolio/positions/[id]/sell/route.ts src/services/domain/position-operations.test.ts` (pass)

## Batch F Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-020` Add-cash now merges into an existing same account+currency cash position (UI modal and chat `add_cash`) instead of always creating a new lot id.

### Verification Executed
- `npx eslint src/components/modals/AddPositionModal.tsx` (pass with pre-existing warnings)
- `npx eslint src/app/api/chat/route.ts` (pass with pre-existing warning)
- `npm run test -- src/services/domain/position-operations.test.ts src/services/domain/action-mapper.test.ts src/services/domain/portfolio-calculator.test.ts src/store/portfolioStore.test.ts src/services/domain/account-role-service.test.ts src/services/domain/cash-account-service.test.ts` (pass)

## Batch G Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-021` Mobile top category tabs now use responsive sizing and horizontal overflow handling.
- `BZ-022` Category asset control rows now keep mobile layout usable via responsive spacer/search sizing.
- `BZ-024` Wallet perp-exchange tags now use semantic warning theme tokens.
- `BZ-025` Donut tooltip typography was raised to a readable baseline.
- `BZ-027` Add Position modal form grids now collapse to one column on mobile.
- `BZ-028` Confirm modal primary CTA label now truncates and no longer overflows.
- `BZ-030` All core modals now support Escape-to-close.
- `BZ-031` Custom Price modal subtitle now truncates long symbol/name combinations.
- `BZ-032` Confirm modal error border now uses semantic negative token instead of hardcoded RGBA.

### Verification Executed
- `npm run test -- src/components/modals/AddPositionModal.test.tsx src/components/modals/ConfirmPositionActionModal.test.tsx src/services/domain/position-operations.test.ts src/services/domain/action-mapper.test.ts src/services/domain/portfolio-calculator.test.ts src/store/portfolioStore.test.ts src/services/domain/account-role-service.test.ts src/services/domain/cash-account-service.test.ts` (pass)
- Note: targeted eslint across UI touched files still surfaces pre-existing repo lint debt (`react-hooks/set-state-in-effect`, `react-hooks/preserve-manual-memoization`) in components not introduced by this batch.

## Batch H Fix Progress (2026-02-13)

### Completed In Code (Pending Manual QA)
- `BZ-023` Global typography now uses app font tokens (removed inline serif overrides, added tokenized display class).
- `BZ-026` Category/exposure colors now use a shared source from `src/lib/colors.ts` across domain + calculator.
- `BZ-029` Confirm action modal labels now use shared modal label typography tokens with readable minimum size.

### Verification Executed
- `npm run test -- src/services/domain/category-service.test.ts src/services/domain/portfolio-calculator.test.ts src/components/modals/ConfirmPositionActionModal.test.tsx` (pass)
- `npx eslint src/components/AppShell.tsx src/services/domain/category-service.ts src/services/domain/portfolio-calculator.ts src/components/modals/ConfirmPositionActionModal.tsx src/services/domain/category-service.test.ts src/lib/colors.ts` (fails only on pre-existing `react-hooks/set-state-in-effect` in `src/components/AppShell.tsx` and existing warnings).

## Batch I QA Verification (2026-02-13)

### Verified In QA
- `BZ-021`, `BZ-022`, `BZ-024`, `BZ-025`, `BZ-027`, `BZ-028`, `BZ-030`, `BZ-031`, `BZ-032`.

### Verification Executed
- Playwright batch audit script: `/tmp/ui_batch_i_verify.js`.
- Command: `/bin/zsh -lc 'NODE_PATH=/Users/fabianwetekamp/billionorzero/node_modules node /tmp/ui_batch_i_verify.js'`.
- QA report artifact: `output/playwright/qa-batch-i-report.json`.
- Additional runtime screenshots:
  - `output/playwright/qa-bz021-home-mobile-batch-i.png`
  - `output/playwright/qa-bz022-crypto-assets-mobile-batch-i.png`
  - `output/playwright/qa-bz024-wallets-desktop.png`
  - `output/playwright/qa-bz025-overview-desktop.png`
  - `output/playwright/qa-bz027-add-position-mobile-batch-i.png`
  - `output/playwright/qa-bz028-bz029-confirm-mobile-batch-i.png`
  - `output/playwright/qa-bz031-custom-price-mobile-batch-i.png`

## Batch J Real-Data QA Verification (2026-02-13)

### Verified In QA
- `BZ-001`, `BZ-002`, `BZ-003`, `BZ-004`, `BZ-005`, `BZ-006`, `BZ-007`, `BZ-011`, `BZ-012`, `BZ-017`, `BZ-018`.

### Verification Executed
- Real-data read-only domain/API verification test:
  - `npm run test -- src/services/domain/real-db-readonly-qa.test.ts`
  - Uses the current `data/db.json` via `readDb()` with no write operations.
- Real-data UI smoke verification (desktop):
  - Script: `/tmp/core_ui_realdb_verify.js`
  - Artifact: `output/playwright/qa-batch-j-core-ui-report.json`
  - Screenshots:
    - `output/playwright/qa-bz001-equities-overview-desktop-batch-j.png`
    - `output/playwright/qa-bz002-equities-assets-desktop-batch-j.png`
    - `output/playwright/qa-bz003-equities-accounts-desktop-batch-j.png`
    - `output/playwright/qa-bz004-crypto-accounts-desktop-batch-j.png`
    - `output/playwright/qa-bz005-crypto-wallets-desktop-batch-j.png`

## Batch K Real-Data QA Verification (2026-02-13)

### Verified In QA
- `BZ-010`, `BZ-013`, `BZ-014`, `BZ-015`, `BZ-016`, `BZ-020`.

### Verification Executed
- Real-data temp-clone API/chat verification:
  - `npm run test -- src/app/api/chat/route.real-db-qa.test.ts src/app/api/portfolio/positions/id.real-db-qa.test.ts src/services/domain/action-mapper.test.ts`
- Full real-data regression rerun:
  - `npm run test -- src/services/domain/action-mapper.test.ts src/app/api/chat/route.real-db-qa.test.ts src/app/api/portfolio/positions/id.real-db-qa.test.ts src/services/domain/real-db-readonly-qa.test.ts src/app/api/portfolio/positions/sell.real-db-qa.test.ts`
- Data safety:
  - All mutation QA used `PORTFOLIO_DB_PATH` temp clones; production `data/db.json` remained unchanged.
...
- Status: `verified` (mapped to `BZ-001`, 2026-02-13)
- Severity: `S2`
...
- Status: `verified` (mapped to `BZ-002`, 2026-02-13)
- Severity: `S1`
...
- Status: `verified` (mapped to `BZ-003`, 2026-02-13)
- Severity: `S1`
...
### Next Fix Order (Historical, Completed)
1. `BUG-003` -> completed via `BZ-003`.
2. `BUG-001` -> completed via `BZ-001`.
3. `BUG-002` -> completed via `BZ-002`.

### Test Cases And Scenarios (For Later Fix Phase)
Status: covered by executed regression/real-data QA in Batches J and K.
...
- QA evidence:
  - `output/playwright/qa-bz001-equities-overview-desktop-batch-j.png`
  - `output/playwright/qa-batch-j-core-ui-report.json`
  - Runtime text probe confirms `3 positions` in equities breakdown.
...
- QA evidence:
  - `output/playwright/qa-bz002-equities-assets-desktop-batch-j.png`
  - `output/playwright/qa-batch-j-core-ui-report.json` (`crmVisible = true`)
...
- QA evidence:
  - `output/playwright/qa-bz003-equities-accounts-desktop-batch-j.png`
  - `output/playwright/qa-batch-j-core-ui-report.json` (`cashMatchCount = 0`)
...
- QA evidence:
  - `output/playwright/qa-bz004-crypto-accounts-desktop-batch-j.png`
  - `output/playwright/qa-batch-j-core-ui-report.json` (`equitySymbolsInCryptoAccounts = []`)
...
- QA evidence:
  - `output/playwright/qa-bz005-crypto-wallets-desktop-batch-j.png`
  - `output/playwright/qa-batch-j-core-ui-report.json` (`equitySymbolsInCryptoWallets = []`)
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-006` test)
  - Real `data/db.json` verification confirms custom prices materially change valuation outputs.
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-007` test)
  - Real `data/db.json` verification confirms fx rates materially change valuation outputs.
...
- QA evidence:
  - `output/playwright/qa-bz008-cash-currency-usd-desktop-batch-j.png`
  - Runtime probe confirms percentage outputs remain in expected range (`7.1%`, `84.4%`, `15.6%`) with no `>100%` inflation.
...
- QA evidence:
  - `output/playwright/qa-bz009-positions-desktop-batch-j.png`
  - `output/playwright/qa-bz008-bz009-report.json` (`hasValueLikeCell = true`).
...
- Status: Verified (2026-02-13)
- Area: Cash -> Positions
- Evidence: `src/app/cash/positions/page.tsx:105`, `src/app/cash/positions/page.tsx:121`, `src/app/cash/positions/page.tsx:236`
- QA evidence:
  - `src/app/api/portfolio/positions/id.real-db-qa.test.ts`
  - Real-data temp-clone test verifies updating/deleting one EUR cash position id does not modify same-currency sibling rows.
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-011/BZ-012` test)
  - Real `data/db.json` includes mixed manual accounts correctly scoped to brokerage and cash views.
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-011/BZ-012` test)
  - Real `data/db.json` role-scope checks include stablecoin-aware cash scope.
...
- Evidence: `src/services/domain/tool-registry.ts:16`, `src/app/api/chat/route.ts:704`, `src/app/api/chat/route.ts:715`
- QA evidence:
  - `src/app/api/chat/route.real-db-qa.test.ts` (`BZ-013` test)
  - Real-data chat route test confirms `buy_position` total-cost flow produces a positive derived amount in pending confirmation action.
...
- Status: Verified (2026-02-13)
- Area: Chat tool execution
- Evidence: `src/app/api/chat/route.ts:859`, `src/app/api/chat/route.ts:880`
- QA evidence:
  - `src/app/api/chat/route.real-db-qa.test.ts` (`BZ-014/BZ-020` test)
  - Real-data chat route test confirms canonical manual account resolution (`revolut broker` -> canonical account id/name).
...
- Status: Verified (2026-02-13)
- Area: Chat tool execution
- Evidence: `src/services/domain/action-mapper.ts:100`, `src/app/api/chat/route.ts:896`, `src/app/api/chat/route.ts:908`, `src/app/api/chat/route.ts:915`, `src/app/api/chat/route.ts:930`
- QA evidence:
  - `src/app/api/chat/route.real-db-qa.test.ts` (`BZ-015` test)
  - `src/services/domain/action-mapper.test.ts` (ambiguous partial account names stay unresolved)
  - Real-data chat route test confirms ambiguous partial account names do not auto-target cash updates.
...
- Evidence: `src/app/api/chat/route.ts:622`, `src/app/api/chat/route.ts:628`, `src/app/api/chat/route.ts:641`, `src/app/api/chat/route.ts:645`
- QA evidence:
  - `src/app/api/chat/route.real-db-qa.test.ts` (`BZ-016` test)
  - Real-data chat route test confirms cash and crypto query counts align with canonical `isPositionInAssetClass` classification.
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-017` test)
  - Real `data/db.json` check confirms API allocation matches canonical calculator allocation for filtered equity set.
...
- QA evidence:
  - `src/services/domain/real-db-readonly-qa.test.ts` (`BZ-018` test)
  - Real `data/db.json` check confirms cash/brokerage filter sets exactly match shared account-role service.
...
- Evidence: `src/app/api/portfolio/positions/[id]/sell/route.ts:4`, `src/app/api/portfolio/positions/[id]/sell/route.ts:38`, `src/app/api/portfolio/positions/[id]/sell/route.ts:46`, `src/services/domain/position-operations.test.ts:6`
- QA evidence:
  - `src/app/api/portfolio/positions/sell.real-db-qa.test.ts`
  - Real-data temp-clone test verifies partial sell updates amount and cost basis proportionally while leaving production `data/db.json` untouched.
- Acceptance criteria: partial sells reduce amount and cost basis proportionally and consistently with domain command logic.

### BZ-020 Add cash flow may always create a new position id for same currency/account
- Severity: Medium
- Status: Verified (2026-02-13)
- Area: Add Position modal
- Evidence: `src/components/modals/AddPositionModal.tsx:191`, `src/components/modals/AddPositionModal.tsx:195`, `src/app/api/chat/route.ts:890`, `src/app/api/chat/route.ts:898`
- QA evidence:
  - `src/app/api/chat/route.real-db-qa.test.ts` (`BZ-014/BZ-020` test)
  - `src/services/domain/action-mapper.test.ts` (`add_cash with account match` resolves existing cash `matchedPositionId`)
  - Real-data chat confirmation payload links existing same account+currency cash position, enabling merge/update path instead of forced new lot id.
...
- Evidence:
  ...
  - Body previously forced Inter/system stack instead of loaded app font variable.
  - AppShell logo and category tabs previously overrode typography with inline Georgia serif.
- Resolution:
  - Body font now uses the loaded Geist token in `src/app/globals.css`.
  - Added tokenized display typography class (`.font-display`) in `src/app/globals.css`.
  - Removed inline serif overrides in `src/components/AppShell.tsx`.
- QA evidence:
  - `output/playwright/qa-bz023-home-desktop.png`
  - `output/playwright/qa-bz023-home-mobile.png`
  - `output/playwright/qa-batch-h-report.json`
- Acceptance criteria:
...
- QA evidence:
  - `output/playwright/qa-bz024-wallets-desktop.png`
  - `output/playwright/qa-batch-i-report.json` (`walletTag.color = rgb(155, 133, 77)`, `walletTag.backgroundColor = rgba(155, 133, 77, 0.08)`)
...
- QA evidence:
  - `src/components/charts/DonutChart.tsx:161` now uses `text-[11px]` tooltip base.
  - `src/components/charts/DonutChart.tsx:179` and `src/components/charts/DonutChart.tsx:185` now use `text-[10px]` (no remaining `text-[8px]`/`text-[9px]` classes).
  - `output/playwright/qa-bz025-overview-desktop.png`
...
- Evidence:
  - `RWA` color previously differed across sources (`#795548` vs `#8D6E63`).
  - Crypto subcategory colors were duplicated in calculator functions.
- Resolution:
  - Added canonical `EXPOSURE_CATEGORY_CONFIG` in `src/lib/colors.ts`.
  - `src/services/domain/category-service.ts` now imports category/subcategory/exposure colors from `src/lib/colors.ts` (single source).
  - `src/services/domain/portfolio-calculator.ts` now uses shared `CRYPTO_COLORS` / `SUBCATEGORY_COLORS` instead of local duplicate maps.
  - Added regression checks in `src/services/domain/category-service.test.ts`.
- QA evidence:
  - `output/playwright/qa-bz026-crypto-assets-desktop.png`
  - `output/playwright/qa-bz026-crypto-exposure-desktop.png`
  - `output/playwright/qa-bz026-crypto-assets-mobile.png`
  - `output/playwright/qa-batch-h-report.json`
- Acceptance criteria:
...
- QA evidence:
  - `output/playwright/qa-bz027-add-position-mobile-batch-i.png`
  - `output/playwright/qa-batch-i-report.json` (`addPositionLayout.stacked = true`)
...
- QA evidence:
  - `output/playwright/qa-bz028-bz029-confirm-mobile-batch-i.png`
  - `output/playwright/qa-batch-i-report.json` (`confirmModal.buttonLabelText = Update Position`, `confirmModal.labelTruncated = false`)
...
- Evidence:
  - Representative labels were rendered with `text-[10px] uppercase` in `src/components/modals/ConfirmPositionActionModal.tsx`.
  - Contrast with larger modal labels in Custom Price modal: `src/components/modals/CustomPriceModal.tsx:117`
- Resolution:
  - Added shared modal typography tokens (`.modal-field-label`, `.modal-field-required`) in `src/app/globals.css`.
  - Replaced tiny 10px label styles in `src/components/modals/ConfirmPositionActionModal.tsx` with shared classes.
- QA evidence:
  - `output/playwright/qa-bz029-confirm-modal-desktop.png`
  - `output/playwright/qa-bz029-confirm-modal-mobile.png`
  - `output/playwright/qa-batch-h-report.json` (`labelMinPx = 12` on desktop and mobile)
- Acceptance criteria:
...
- QA evidence:
  - `output/playwright/qa-batch-i-report.json`:
    - `desktop.escapeClose.addPosition = true`
    - `desktop.escapeClose.addWallet = true`
    - `desktop.escapeClose.customPrice = true`
    - `desktop.escapeClose.confirmAction = true`
...
- QA evidence:
  - `output/playwright/qa-bz031-custom-price-mobile-batch-i.png`
  - `output/playwright/qa-batch-i-report.json` (`customPrice.truncated = true`)
...
- QA evidence:
  - `src/components/modals/ConfirmPositionActionModal.tsx:1767` uses `border-[var(--negative)]`.
  - No remaining `rgba(201,123,123` usage in modal component.
